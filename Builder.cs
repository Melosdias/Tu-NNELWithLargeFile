...
public class Builder : Unitees
{
    ...
    private void Update()
    {
        if (this.Health <= 0)
        {
            photonView.RPC("UpdateLive", RpcTarget.All, 6);
        }
        if (task == true)
        {
            if (Vector3.Distance(hit.point, this.gameObject.GetComponent<NavMeshAgent>().transform.position)<3)
            {
                task = false;
                an.SetBool("isWalking", false);
                this.gameObject.GetComponent<NavMeshAgent>().isStopped = true;
                this.gameObject.GetComponent<NavMeshAgent>().ResetPath();
                an.SetBool("isWorking", true);
